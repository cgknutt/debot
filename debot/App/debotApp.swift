...
struct debotApp: App {
    ...
    private func configureGlobalFonts() {
        ...
        guard let font = titanOneFont else {
            ...
            if let font = UIFont(name: fontName, size: 17) {
                print("✅ Font registration succeeded! Font is now available: \(font.fontName)")
                return
            }
            
            print("❌ Font registration failed. Using system font instead.")
            return
        }
    ...
    // Helper function to manually register the font from the Assets folder
